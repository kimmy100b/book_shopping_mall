...
<script>
	function search_member(search_period) {
		...
		formObj.appendChild(beginDate);
		formObj.appendChild(endDate);
	...
	function fn_enable_detail_search(r_search) {
		if (r_search.value == 'detail_search') {
			$('#tr_detail_search1').show();
			$('#tr_detail_search2').show();
			$('#tr_simple_search').hide();
			
		} else {
			$('#tr_simple_search').show();
			$('#tr_detail_search1').hide();
			$('#tr_detail_search2').hide();
		}
	}
			...
			<tbody>
				<tr>
					<td>
						<input type="radio" name="r_search_option" value="simple_search"
							checked onClick="fn_enable_detail_search(this)"
						/>
						간단조회 &nbsp;&nbsp;&nbsp;
				...
				<tr id="tr_simple_search">
					<td>
						...
						일 &nbsp;이전&nbsp;&nbsp;&nbsp;&nbsp; <a
							href="javascript:search_member('today')"
						> <img
				...
				<tr id="tr_detail_search2">
					<td>
						...
						<input type="button" value="조회" name="btn_search"
							onClick="fn_detail_search()" disabled
						/>
					</td>
				</tr>
			</tbody>
					...
					<c:otherwise>
						<c:forEach var="item" items="${member_list}" varStatus="item_num">
							<tr>
								...
								<td width=50%>
									<strong>${item.roadAddress}</strong><br> <strong>${item.jibunAddress}</strong><br>
					...
					<td colspan=8 class="fixed">
						<c:forEach var="page" begin="1" end="10" step="1">
							<c:if test="${section >1 && page==1 }">
								<a
									href="${pageContext.request.contextPath}/admin/member/adminMemberMain.do?section=${section-1}&pageNum=${(section-1)*10 +1 }"
								>&nbsp;pre &nbsp;</a>
							</c:if>
							<a
								href="${pageContext.request.contextPath}/admin/member/adminMemberMain.do?section=${section}&pageNum=${page}"
							>${(section-1)*10 +page } </a>
							<c:if test="${page ==10 }">
								<a
